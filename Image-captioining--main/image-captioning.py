...
# Update base path to new location
base_path = Path('/kaggle/input/coco-image-caption')
annot = base_path / 'annotations_trainval2014' / 'annotations' / 'captions_train2014.json'
...
# Prepare samples with the new image path structure
samples = []
for sample in data:
    im = 'COCO_train2014_%012d.jpg' % sample['image_id']
    samples.append([im, sample['caption']])

# Create DataFrame and update image paths
df = pd.DataFrame(samples, columns=['image', 'caption'])
df['image'] = df['image'].apply(
    lambda x: base_path / 'train2014' / 'train2014' / x
)

# Sample 150,000 entries and reset index
...
# Initialize BLEU scores list to store results
bleu_scores = []
...
for i in range(50):
    ...
    reference = [test_caption.split()]  # The reference caption as a list of words
    candidate = gen_caption.split()    # The generated caption as a list of words
    bleu_score = sentence_bleu(reference, candidate)
    
    # Append the score
    bleu_scores.append(bleu_score)
    
    # Plot the image and the captions
    plt.title(f"actual: {test_caption}\nmodel: {gen_caption}\ntemp: {t} deterministic: {det}")
    plt.axis('off')
    plt.show()
